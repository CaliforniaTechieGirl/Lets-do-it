...
export default function MapView({ ideas, onSelect }) {
    ...
    markersRef.current = ideas.map((idea, idx) => {
      ...
      const svg = `
        <svg xmlns="http://www.w3.org/2000/svg" width="32" height="40" viewBox="0 0 32 40">
          <path d="M16 0C7.16 0 0 7.16 0 16c0 10 16 24 16 24S32 26 32 16C32 7.16 24.84 0 16 0z" fill="${T.primary}"/>
          <circle cx="16" cy="16" r="11" fill="white"/>
          <text x="16" y="21" text-anchor="middle" font-family="Plus Jakarta Sans, system-ui, sans-serif" font-size="${num > 9 ? 10 : 12}" font-weight="600" fill="${T.primary}">${num}</text>
        </svg>`;
  ...
  return (
    <div>
      {/* Map */}
      <div ref={mapRef} style={{ width: "100%", height: 380, borderRadius: T.radiusLg, overflow: "hidden", border: `1px solid ${T.surfaceMid}`, marginBottom: 14 }} />

      {/* Selected idea card */}
      {selIdea && (() => {
        const num = ideas.findIndex(i => i.id === selIdea.id) + 1;
        return (
          <div style={{ padding: 14, background: T.surface, border: `1.5px solid ${T.primaryMid}`, borderRadius: T.radiusMd, marginBottom: 14 }}>
            <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: "flex", gap: 7 }}>
                  ...
                  <button onClick={() => { setSel(null); onSelect(selIdea); }} style={{ fontFamily: T.fontFamily, fontSize: 11, padding: "5px 11px", borderRadius: T.radius, background: T.primaryLight, color: T.primary, border: `1px solid ${T.primaryMid}`, cursor: "pointer", fontWeight: 500 }}>
                    See Details
                  </button>
                  <button onClick={() => setSel(null)} style={{ fontFamily: T.fontFamily, fontSize: 11, padding: "5px 11px", borderRadius: T.radius, background: T.bg, color: T.textMuted, border: `1px solid ${T.surfaceMid}`, cursor: "pointer", fontWeight: 500 }}>
                    Close
                  </button>
                </div>
              </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
        {ideas.map((idea, idx) => (
          <button
            ...
            style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", background: sel === idea.id ? T.primaryLight : T.surface, border: `1px solid ${sel === idea.id ? T.primaryMid : T.surfaceMid}`, borderRadius: T.radius, cursor: "pointer", textAlign: "left", fontFamily: T.fontFamily, transition: "all 0.1s" }}
          >
            <div style={{ width: 22, height: 22, borderRadius: "50%", background: sel === idea.id ? T.primary : T.primaryLight, color: sel === idea.id ? "#fff" : T.primary, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, flexShrink: 0 }}>
              {idx + 1}
            </div>
